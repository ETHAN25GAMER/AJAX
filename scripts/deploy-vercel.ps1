...
Write-Host "3. Supabase: confirm all migrations are applied, incl. 0009_deployment_tier.sql"
Write-Host "   ('select * from deployment_settings;' should return one tier2 row)."
Write-Host "4. Smoke-test from your phone: 'Hi' -> greeting; price question -> quote tool;"
Write-Host "   photo -> identify_pest. First reply confirms Fable 5 + key are live."
Write-Host "5. Known gap: /api/cron/amc exists but is NOT scheduled in vercel.json -"
Write-Host "   AMC outreach will not fire until you add it."
